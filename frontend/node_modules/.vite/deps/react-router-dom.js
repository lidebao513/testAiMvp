...
//#region node_modules/react-router/node_modules/@remix-run/router/dist/router.js
...
function createMemoryHistory(options) {
	...
	function createMemoryLocation(to, state, key) {
		...
		let location = createLocation$1(entries ? getCurrentLocation().pathname : "/", to, state, key);
...
var immutableRouteKeys$1 = new Set([
	"lazy",
	"caseSensitive",
	"path",
	"id",
...
function convertRoutesToDataRoutes$1(routes, mapRouteProperties, parentPath, manifest) {
	...
	return routes.map((route, index) => {
		...
		if (isIndexRoute$1(route)) {
			let indexRoute = _extends$3({}, route, mapRouteProperties(route), { id });
...
function matchRoutes(routes, locationArg, basename) {
	...
	return matchRoutesImpl$1(routes, locationArg, basename, false);
}
function matchRoutesImpl$1(routes, locationArg, basename, allowPartial) {
	let pathname = stripBasename$1((typeof locationArg === "string" ? parsePath(locationArg) : locationArg).pathname || "/", basename);
	if (pathname == null) return null;
	let branches = flattenRoutes$1(routes);
	rankRouteBranches$1(branches);
	let matches = null;
	for (let i = 0; matches == null && i < branches.length; ++i) {
		let decoded = decodePath$1(pathname);
		matches = matchRouteBranch$1(branches[i], decoded, allowPartial);
	}
	return matches;
}
function convertRouteMatchToUiMatch$1(match, loaderData) {
...
function flattenRoutes$1(routes, branches, parentsMeta, parentPath) {
	...
	let flattenRoute = (route, index, relativePath) => {
		...
		let path = joinPaths$1([parentPath, meta.relativePath]);
		...
		if (route.children && route.children.length > 0) {
			...
			flattenRoutes$1(route.children, branches, routesMeta, path);
		}
		if (route.path == null && !route.index) return;
		branches.push({
			path,
			score: computeScore$1(path, route.index),
			routesMeta
		});
	};
	routes.forEach((route, index) => {
		...
		else for (let exploded of explodeOptionalSegments$1(route.path)) flattenRoute(route, index, exploded);
	});
	return branches;
}
/**
...
*/
function explodeOptionalSegments$1(path) {
	...
	let restExploded = explodeOptionalSegments$1(rest.join("/"));
	let result = [];
	result.push(...restExploded.map((subpath) => subpath === "" ? required : [required, subpath].join("/")));
	if (isOptional) result.push(...restExploded);
	return result.map((exploded) => path.startsWith("/") && exploded === "" ? "/" : exploded);
}
function rankRouteBranches$1(branches) {
	branches.sort((a, b) => a.score !== b.score ? b.score - a.score : compareIndexes$1(a.routesMeta.map((meta) => meta.childrenIndex), b.routesMeta.map((meta) => meta.childrenIndex)));
}
var paramRe$1 = /^:[\w-]+$/;
var dynamicSegmentValue$1 = 3;
var indexRouteValue$1 = 2;
var emptySegmentValue$1 = 1;
var staticSegmentValue$1 = 10;
var splatPenalty$1 = -2;
var isSplat$1 = (s) => s === "*";
function computeScore$1(path, index) {
	let segments = path.split("/");
	let initialScore = segments.length;
	if (segments.some(isSplat$1)) initialScore += splatPenalty$1;
	if (index) initialScore += indexRouteValue$1;
	return segments.filter((s) => !isSplat$1(s)).reduce((score, segment) => score + (paramRe$1.test(segment) ? dynamicSegmentValue$1 : segment === "" ? emptySegmentValue$1 : staticSegmentValue$1), initialScore);
}
function compareIndexes$1(a, b) {
	return a.length === b.length && a.slice(0, -1).every((n, i) => n === b[i]) ? a[a.length - 1] - b[b.length - 1] : 0;
}
function matchRouteBranch$1(branch, pathname, allowPartial) {
	...
	for (let i = 0; i < routesMeta.length; ++i) {
		...
		matches.push({
			...
			pathname: joinPaths$1([matchedPathname, match.pathname]),
			pathnameBase: normalizePathname$1(joinPaths$1([matchedPathname, match.pathnameBase])),
			route
		});
		if (match.pathnameBase !== "/") matchedPathname = joinPaths$1([matchedPathname, match.pathnameBase]);
	}
	return matches;
}
/**
...
var ABSOLUTE_URL_REGEX$1$3 = /^(?:[a-z][a-z0-9+.-]*:|\/\/)/i;
var isAbsoluteUrl$1 = (url) => ABSOLUTE_URL_REGEX$1$3.test(url);
/**
...
function resolvePath(to, fromPathname) {
	...
	let pathname;
	if (toPathname) if (isAbsoluteUrl$1(toPathname)) pathname = toPathname;
	else {
		...
		if (toPathname.startsWith("/")) pathname = resolvePathname$1(toPathname.substring(1), "/");
		else pathname = resolvePathname$1(toPathname, fromPathname);
	}
	else pathname = fromPathname;
	return {
		pathname,
		search: normalizeSearch$1(search),
...
function getInvalidPathError$1(char, field, dest, path) {
...
function getResolveToMatches$1(matches, v7_relativeSplatPath) {
	let pathMatches = getPathContributingMatches$1(matches);
...
* @private
*/
var joinPaths$1 = (paths) => paths.join("/").replace(/\/\/+/g, "/");
/**
* @private
*/
var normalizePathname$1 = (pathname) => pathname.replace(/\/+$/, "").replace(/^\/*/, "/");
/**
* @private
*/
var normalizeSearch$1 = (search) => !search || search === "?" ? "" : search.startsWith("?") ? search : "?" + search;
/**
* @private
*/
var normalizeHash$1 = (hash) => !hash || hash === "#" ? "" : hash.startsWith("#") ? hash : "#" + hash;
/**
...
var json = function json(data, init) {
	...
	return new Response(JSON.stringify(data), _extends$3({}, responseInit, { headers }));
};
...
var redirect = function redirect(url, init) {
	...
	return new Response(null, _extends$3({}, responseInit, { headers }));
};
/**
...
var validMutationMethodsArr$1 = [
	"post",
	"put",
	"patch",
	"delete"
];
var validMutationMethods$1 = new Set(validMutationMethodsArr$1);
var validRequestMethodsArr$1 = ["get", ...validMutationMethodsArr$1];
var validRequestMethods$1 = new Set(validRequestMethodsArr$1);
var redirectStatusCodes$1 = new Set([
	301,
	302,
	303,
	307,
...
var redirectPreserveMethodStatusCodes$1 = new Set([307, 308]);
var IDLE_NAVIGATION$1 = {
	state: "idle",
	location: void 0,
	formMethod: void 0,
	formAction: void 0,
...
var ABSOLUTE_URL_REGEX$4 = /^(?:[a-z][a-z0-9+.-]*:|\/\/)/i;
var defaultMapRouteProperties$1 = (route) => ({ hasErrorBoundary: Boolean(route.hasErrorBoundary) });
var TRANSITIONS_STORAGE_KEY$1 = "remix-router-transitions";
/**
* Create a router and listen to history POP navigations
*/
function createRouter$1(init) {
	...
	} else mapRouteProperties = defaultMapRouteProperties$1;
	let manifest = {};
	let dataRoutes = convertRoutesToDataRoutes$1(init.routes, mapRouteProperties, void 0, manifest);
	let inFlightDataRoutes;
	let basename = init.basename || "/";
	let dataStrategyImpl = init.dataStrategy || defaultDataStrategy$1;
	...
	if (initialMatches == null && !patchRoutesOnNavigationImpl) {
		let error = getInternalRouterError$1(404, { pathname: init.history.location.pathname });
		let { matches, route } = getShortCircuitMatches$1(dataRoutes);
	...
	if (!initialMatches) {
		...
		if (errors) {
			...
			initialized = initialMatches.slice(0, idx + 1).every((m) => !shouldLoadRouteOnHydration$1(m.route, loaderData, errors));
		} else initialized = initialMatches.every((m) => !shouldLoadRouteOnHydration$1(m.route, loaderData, errors));
	...
	function initialize() {
			...
			if (blockerKey && delta != null) {
				...
				updateBlocker(blockerKey, {
					...
					reset() {
						...
						blockers.set(blockerKey, IDLE_BLOCKER$1);
		...
		if (isBrowser) {
			restoreAppliedTransitions$1(routerWindow, appliedViewTransitions);
			let _saveAppliedTransitions = () => persistAppliedTransitions$1(routerWindow, appliedViewTransitions);
	...
	function completeNavigation(location, newState, _temp) {
		...
		let isActionReload = state.actionData != null && state.navigation.formMethod != null && isMutationMethod$1(state.navigation.formMethod) && state.navigation.state === "loading" && ((_location$state = location.state) == null ? void 0 : _location$state._isRedirect) !== true;
		let actionData;
		...
		let loaderData = newState.loaderData ? mergeLoaderData$1(state.loaderData, newState.loaderData, newState.matches || [], newState.errors) : state.loaderData;
		let blockers = state.blockers;
		if (blockers.size > 0) {
			blockers = new Map(blockers);
			blockers.forEach((_, k) => blockers.set(k, IDLE_BLOCKER$1));
		}
		let preventScrollReset = pendingPreventScrollReset === true || state.navigation.formMethod != null && isMutationMethod$1(state.navigation.formMethod) && ((_location$state2 = location.state) == null ? void 0 : _location$state2._isRedirect) !== true;
	...
	async function navigate(to, opts) {
		...
		let normalizedPath = normalizeTo$1(state.location, state.matches, basename, future.v7_prependBasename, to, future.v7_relativeSplatPath, opts == null ? void 0 : opts.fromRouteId, opts == null ? void 0 : opts.relative);
		let { path, submission, error } = normalizeNavigateOptions$1(future.v7_normalizeFormMethod, false, normalizedPath, opts);
		let currentLocation = state.location;
		let nextLocation = createLocation$1(state.location, path, opts && opts.state);
		...
		else if (submission != null && isMutationMethod$1(submission.formMethod) && submission.formAction === state.location.pathname + state.location.search) historyAction = Action.Replace;
	...
	async function startNavigation(historyAction, location, opts) {
		...
		let flushSync = (opts && opts.flushSync) === true;
		if (matches && state.initialized && !isRevalidationRequired && isHashChangeOnly$1(state.location, location) && !(opts && opts.submission && isMutationMethod$1(opts.submission.formMethod))) {
		...
		let request = createClientSideRequest$1(init.history, location, pendingNavigationController.signal, opts && opts.submission);
		let pendingActionResult;
		if (opts && opts.pendingError) pendingActionResult = [findNearestBoundary$1(matches).route.id, {
			type: ResultType$1.error,
			error: opts.pendingError
		}];
		else if (opts && opts.submission && isMutationMethod$1(opts.submission.formMethod)) {
			...
			if (actionResult.pendingActionResult) {
				...
				if (isErrorResult$1(result) && isRouteErrorResponse(result.error) && result.error.status === 404) {
					pendingNavigationController = null;
					completeNavigation(location, {
			...
			loadingNavigation = getLoadingNavigation$1(location, opts.submission);
			flushSync = false;
			fogOfWar.active = false;
			request = createClientSideRequest$1(init.history, request.url, request.signal);
		}
		...
		pendingNavigationController = null;
		completeNavigation(location, _extends$3({ matches: updatedMatches || matches }, getActionDataForCommit$1(pendingActionResult), {
			loaderData,
			errors
		}));
	}
	async function handleAction(request, location, submission, matches, isFogOfWar, opts) {
		if (opts === void 0) opts = {};
		interruptActiveLoads();
		updateState({ navigation: getSubmittingNavigation$1(location, submission) }, { flushSync: opts.flushSync === true });
		if (isFogOfWar) {
			let discoverResult = await discoverRoutes(matches, location.pathname, request.signal);
			if (discoverResult.type === "aborted") return { shortCircuited: true };
			else if (discoverResult.type === "error") {
				let boundaryId = findNearestBoundary$1(discoverResult.partialMatches).route.id;
				return {
		...
		let result;
		let actionMatch = getTargetMatch$1(matches, location);
		if (!actionMatch.route.action && !actionMatch.route.lazy) result = {
			type: ResultType$1.error,
			error: getInternalRouterError$1(405, {
		...
		if (isRedirectResult$1(result)) {
			let replace;
			if (opts && opts.replace != null) replace = opts.replace;
			else replace = normalizeRedirectLocation$1(result.response.headers.get("Location"), new URL(request.url), basename, init.history) === state.location.pathname + state.location.search;
		...
		if (isDeferredResult$1(result)) throw getInternalRouterError$1(400, { type: "defer-action" });
		if (isErrorResult$1(result)) {
			let boundaryMatch = findNearestBoundary$1(matches, actionMatch.route.id);
	...
	async function handleLoaders(request, location, matches, isFogOfWar, overrideNavigation, submission, fetcherSubmission, replace, initialHydration, flushSync, pendingActionResult) {
		let loadingNavigation = overrideNavigation || getLoadingNavigation$1(location, submission);
		let activeSubmission = submission || fetcherSubmission || getSubmissionFromNavigation$1(loadingNavigation);
		let shouldUpdateNavigationState = !isUninterruptedRevalidation && (!future.v7_partialHydration || !initialHydration);
		if (isFogOfWar) {
			if (shouldUpdateNavigationState) {
				let actionData = getUpdatedActionData(pendingActionResult);
				updateState(_extends$3({ navigation: loadingNavigation }, actionData !== void 0 ? { actionData } : {}), { flushSync });
			}
			let discoverResult = await discoverRoutes(matches, location.pathname, request.signal);
			if (discoverResult.type === "aborted") return { shortCircuited: true };
			else if (discoverResult.type === "error") {
		...
		let routesToUse = inFlightDataRoutes || dataRoutes;
		let [matchesToLoad, revalidatingFetchers] = getMatchesToLoad$1(init.history, state, matches, activeSubmission, location, future.v7_partialHydration && initialHydration === true, future.v7_skipActionErrorRevalidation, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, deletedFetchers, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionResult);
		...
		if (matchesToLoad.length === 0 && revalidatingFetchers.length === 0) {
			...
			completeNavigation(location, _extends$3({
				matches,
				loaderData: {},
				errors: pendingActionResult && isErrorResult$1(pendingActionResult[1]) ? { [pendingActionResult[0]]: pendingActionResult[1].error } : null
			}, getActionDataForCommit$1(pendingActionResult), updatedFetchers ? { fetchers: new Map(state.fetchers) } : {}), { flushSync });
			return { shortCircuited: true };
		}
		if (shouldUpdateNavigationState) {
			let updates = {};
		...
		let redirect = findRedirect$1(loaderResults);
		if (redirect) {
			await startRedirectNavigation(request, redirect.result, true, { replace });
			return { shortCircuited: true };
		}
		redirect = findRedirect$1(fetcherResults);
		if (redirect) {
			fetchRedirectIds.add(redirect.key);
			await startRedirectNavigation(request, redirect.result, true, { replace });
			return { shortCircuited: true };
		}
		let { loaderData, errors } = processLoaderData$1(state, matches, loaderResults, pendingActionResult, revalidatingFetchers, fetcherResults, activeDeferreds);
		activeDeferreds.forEach((deferredData, routeId) => {
			deferredData.subscribe((aborted) => {
				if (aborted || deferredData.done) activeDeferreds.delete(routeId);
			});
		});
		if (future.v7_partialHydration && initialHydration && state.errors) errors = _extends$3({}, state.errors, errors);
		let updatedFetchers = markFetchRedirectsDone();
	...
	function getUpdatedActionData(pendingActionResult) {
		if (pendingActionResult && !isErrorResult$1(pendingActionResult[1])) return { [pendingActionResult[0]]: pendingActionResult[1].data };
	...
	function getUpdatedRevalidatingFetchers(revalidatingFetchers) {
		revalidatingFetchers.forEach((rf) => {
			...
			let revalidatingFetcher = getLoadingFetcher$1(void 0, fetcher ? fetcher.data : void 0);
	...
	function fetch(key, routeId, href, opts) {
		...
		let flushSync = (opts && opts.flushSync) === true;
		let routesToUse = inFlightDataRoutes || dataRoutes;
		let normalizedPath = normalizeTo$1(state.location, state.matches, basename, future.v7_prependBasename, href, future.v7_relativeSplatPath, routeId, opts == null ? void 0 : opts.relative);
		let matches = matchRoutes(routesToUse, normalizedPath, basename);
		let fogOfWar = checkFogOfWar(matches, routesToUse, normalizedPath);
		if (fogOfWar.active && fogOfWar.matches) matches = fogOfWar.matches;
		if (!matches) {
			setFetcherError(key, routeId, getInternalRouterError$1(404, { pathname: normalizedPath }), { flushSync });
			return;
		}
		let { path, submission, error } = normalizeNavigateOptions$1(future.v7_normalizeFormMethod, true, normalizedPath, opts);
		if (error) {
			setFetcherError(key, routeId, error, { flushSync });
			return;
		}
		let match = getTargetMatch$1(matches, path);
		let preventScrollReset = (opts && opts.preventScrollReset) === true;
		if (submission && isMutationMethod$1(submission.formMethod)) {
	...
	async function handleFetcherAction(key, routeId, path, match, requestMatches, isFogOfWar, flushSync, preventScrollReset, submission) {
		...
		function detectAndHandle405Error(m) {
			if (!m.route.action && !m.route.lazy) {
				setFetcherError(key, routeId, getInternalRouterError$1(405, {
		...
		updateFetcherState(key, getSubmittingFetcher$1(submission, state.fetchers.get(key)), { flushSync });
		let abortController = new AbortController();
		let fetchRequest = createClientSideRequest$1(init.history, path, abortController.signal, submission);
		if (isFogOfWar) {
			...
			if (discoverResult.type === "aborted") return;
				...
				setFetcherError(key, routeId, getInternalRouterError$1(404, { pathname: path }), { flushSync });
				return;
			} else {
				requestMatches = discoverResult.matches;
				match = getTargetMatch$1(requestMatches, path);
		...
		if (future.v7_fetcherPersist && deletedFetchers.has(key)) {
			if (isRedirectResult$1(actionResult) || isErrorResult$1(actionResult)) {
				updateFetcherState(key, getDoneFetcher$1(void 0));
				return;
			}
		} else {
			if (isRedirectResult$1(actionResult)) {
				...
				if (pendingNavigationLoadId > originatingLoadId) {
					...
					updateFetcherState(key, getLoadingFetcher$1(submission));
			...
			if (isErrorResult$1(actionResult)) {
				setFetcherError(key, routeId, actionResult.error);
				return;
			}
		}
		if (isDeferredResult$1(actionResult)) throw getInternalRouterError$1(400, { type: "defer-action" });
		let nextLocation = state.navigation.location || state.location;
		let revalidationRequest = createClientSideRequest$1(init.history, nextLocation, abortController.signal);
		let routesToUse = inFlightDataRoutes || dataRoutes;
		let matches = state.navigation.state !== "idle" ? matchRoutes(routesToUse, state.navigation.location, basename) : state.matches;
		invariant$1(matches, "Didn't find any matches after fetcher action");
		let loadId = ++incrementingLoadId;
		fetchReloadIds.set(key, loadId);
		let loadFetcher = getLoadingFetcher$1(submission, actionResult.data);
		state.fetchers.set(key, loadFetcher);
		let [matchesToLoad, revalidatingFetchers] = getMatchesToLoad$1(init.history, state, matches, submission, nextLocation, false, future.v7_skipActionErrorRevalidation, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, deletedFetchers, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, [match.route.id, actionResult]);
		revalidatingFetchers.filter((rf) => rf.key !== key).forEach((rf) => {
			let staleKey = rf.key;
			let existingFetcher = state.fetchers.get(staleKey);
			let revalidatingFetcher = getLoadingFetcher$1(void 0, existingFetcher ? existingFetcher.data : void 0);
		...
		let { loaderData, errors } = processLoaderData$1(state, matches, loaderResults, void 0, revalidatingFetchers, fetcherResults, activeDeferreds);
		if (state.fetchers.has(key)) {
			let doneFetcher = getDoneFetcher$1(actionResult.data);
		...
		if (state.navigation.state === "loading" && loadId > pendingNavigationLoadId) {
			...
			updateState({
				errors,
				loaderData: mergeLoaderData$1(state.loaderData, loaderData, matches, errors),
				fetchers: new Map(state.fetchers)
			});
			isRevalidationRequired = false;
		}
	...
	async function handleFetcherLoader(key, routeId, path, match, matches, isFogOfWar, flushSync, preventScrollReset, submission) {
		...
		updateFetcherState(key, getLoadingFetcher$1(submission, existingFetcher ? existingFetcher.data : void 0), { flushSync });
		let abortController = new AbortController();
		let fetchRequest = createClientSideRequest$1(init.history, path, abortController.signal);
		if (isFogOfWar) {
			...
			if (discoverResult.type === "aborted") return;
				...
				match = getTargetMatch$1(matches, path);
			}
		}
		fetchControllers.set(key, abortController);
		let originatingLoadId = incrementingLoadId;
		let result = (await callDataStrategy("loader", state, fetchRequest, [match], matches, key))[match.route.id];
		if (isDeferredResult$1(result)) result = await resolveDeferredData$1(result, fetchRequest.signal, true) || result;
		if (fetchControllers.get(key) === abortController) fetchControllers.delete(key);
		...
		if (isRedirectResult$1(result)) if (pendingNavigationLoadId > originatingLoadId) {
			updateFetcherState(key, getDoneFetcher$1(void 0));
			return;
		} else {
			fetchRedirectIds.add(key);
		...
		invariant$1(!isDeferredResult$1(result), "Unhandled fetcher deferred data");
		updateFetcherState(key, getDoneFetcher$1(result.data));
	}
	/**
	...
	*/
	async function startRedirectNavigation(request, redirect, isNavigation, _temp2) {
		...
		let redirectLocation = createLocation$1(state.location, location, { _isRedirect: true });
		if (isBrowser) {
			let isDocumentReload = false;
			if (redirect.response.headers.has("X-Remix-Reload-Document")) isDocumentReload = true;
			else if (ABSOLUTE_URL_REGEX$4.test(location)) {
				const url = init.history.createURL(location);
				isDocumentReload = url.origin !== routerWindow.location.origin || stripBasename$1(url.pathname, basename) == null;
			}
			if (isDocumentReload) {
		...
		if (!submission && !fetcherSubmission && formMethod && formAction && formEncType) submission = getSubmissionFromNavigation$1(state.navigation);
		let activeSubmission = submission || fetcherSubmission;
		if (redirectPreserveMethodStatusCodes$1.has(redirect.response.status) && activeSubmission && isMutationMethod$1(activeSubmission.formMethod)) await startNavigation(redirectHistoryAction, redirectLocation, {
			submission: _extends$3({}, activeSubmission, { formAction: location }),
			preventScrollReset: preventScrollReset || pendingPreventScrollReset,
			enableViewTransition: isNavigation ? pendingViewTransitionEnabled : void 0
		});
		else await startNavigation(redirectHistoryAction, redirectLocation, {
			overrideNavigation: getLoadingNavigation$1(redirectLocation, submission),
			fetcherSubmission,
			preventScrollReset: preventScrollReset || pendingPreventScrollReset,
			enableViewTransition: isNavigation ? pendingViewTransitionEnabled : void 0
		});
	...
	async function callDataStrategy(type, state, request, matchesToLoad, matches, fetcherKey) {
		...
		let dataResults = {};
		try {
			results = await callDataStrategyImpl$1(dataStrategyImpl, type, state, request, matchesToLoad, matches, fetcherKey, manifest, mapRouteProperties);
		} catch (e) {
		...
		for (let [routeId, result] of Object.entries(results)) if (isRedirectDataStrategyResultResult$1(result)) {
			let response = result.result;
			dataResults[routeId] = {
				type: ResultType$1.redirect,
				response: normalizeRelativeRoutingRedirectResponse$1(response, request, routeId, matches, basename, future.v7_relativeSplatPath)
			};
		} else dataResults[routeId] = await convertDataStrategyResultToDataResult$1(result);
		return dataResults;
	}
	async function callLoadersAndMaybeResolveData(state, matches, matchesToLoad, fetchersToLoad, request) {
		...
		let fetcherResultsPromise = Promise.all(fetchersToLoad.map(async (f) => {
			if (f.matches && f.match && f.controller) {
				let result = (await callDataStrategy("loader", state, createClientSideRequest$1(init.history, f.path, f.controller.signal), [f.match], f.matches, f.key))[f.match.route.id];
				return { [f.key]: result };
			} else return Promise.resolve({ [f.key]: {
				type: ResultType$1.error,
				error: getInternalRouterError$1(404, { pathname: f.path })
			} });
		}));
		let loaderResults = await loaderResultsPromise;
		let fetcherResults = (await fetcherResultsPromise).reduce((acc, r) => Object.assign(acc, r), {});
		await Promise.all([resolveNavigationDeferredResults$1(matches, loaderResults, request.signal, currentMatches, state.loaderData), resolveFetcherDeferredResults$1(matches, fetcherResults, fetchersToLoad)]);
		return {
			loaderResults,
			fetcherResults
		};
	...
	function setFetcherError(key, routeId, error, opts) {
		if (opts === void 0) opts = {};
		let boundaryMatch = findNearestBoundary$1(state.matches, routeId);
		deleteFetcher(key);
		updateState({
	...
	function getFetcher(key) {
		...
		return state.fetchers.get(key) || IDLE_FETCHER$1;
	}
	...
	function markFetchersDone(keys) {
		for (let key of keys) {
			let doneFetcher = getDoneFetcher$1(getFetcher(key).data);
			state.fetchers.set(key, doneFetcher);
		}
	}
	...
	function getBlocker(key, fn) {
		let blocker = state.blockers.get(key) || IDLE_BLOCKER$1;
	...
	function handleNavigational404(pathname) {
		let error = getInternalRouterError$1(404, { pathname });
		let { matches, route } = getShortCircuitMatches$1(inFlightDataRoutes || dataRoutes);
	...
	function enableScrollRestoration(positions, getPosition, getKey) {
		...
		if (!initialScrollRestored && state.navigation === IDLE_NAVIGATION$1) {
	...
	function getScrollKey(location, matches) {
		if (getScrollRestorationKey) return getScrollRestorationKey(location, matches.map((m) => convertRouteMatchToUiMatch$1(m, state.loaderData))) || location.key;
		return location.key;
	}
	...
	function checkFogOfWar(matches, routesToUse, pathname) {
		if (patchRoutesOnNavigationImpl) {
			if (!matches) return {
				active: true,
				matches: matchRoutesImpl$1(routesToUse, pathname, basename, true) || []
			};
	...
	async function discoverRoutes(matches, pathname, signal, fetcherKey) {
		...
		while (true) {
			...
			try {
				await patchRoutesOnNavigationImpl({
					...
					patch: (routeId, children) => {
						...
						patchRoutesImpl$1(routeId, children, routesToUse, localManifest, mapRouteProperties);
					}
				});
			} catch (e) {
				return {
			...
			let newPartialMatches = matchRoutesImpl$1(routesToUse, pathname, basename, true);
	...
	function patchRoutes(routeId, children) {
		let isNonHMR = inFlightDataRoutes == null;
		patchRoutesImpl$1(routeId, children, inFlightDataRoutes || dataRoutes, manifest, mapRouteProperties);
		if (isNonHMR) {
...
function isSubmissionNavigation$1(opts) {
	return opts != null && ("formData" in opts && opts.formData != null || "body" in opts && opts.body !== void 0);
}
function normalizeTo$1(location, matches, basename, prependBasename, to, v7_relativeSplatPath, fromRouteId, relative) {
	...
	let path = resolveTo$1(to ? to : ".", getResolveToMatches$1(contextualMatches, v7_relativeSplatPath), stripBasename$1(location.pathname, basename) || location.pathname, relative === "path");
	if (to == null) {
		path.search = location.search;
		path.hash = location.hash;
	}
	if ((to == null || to === "" || to === ".") && activeRouteMatch) {
		let nakedIndex = hasNakedIndexQuery$1(path.search);
	...
	if (prependBasename && basename !== "/") path.pathname = path.pathname === "/" ? basename : joinPaths$1([basename, path.pathname]);
	return createPath(path);
}
function normalizeNavigateOptions$1(normalizeFormMethod, isFetcher, path, opts) {
	if (!opts || !isSubmissionNavigation$1(opts)) return { path };
	if (opts.formMethod && !isValidMethod$1(opts.formMethod)) return {
		path,
		error: getInternalRouterError$1(405, { method: opts.formMethod })
	};
	let getInvalidBodyError = () => ({
		path,
		error: getInternalRouterError$1(400, { type: "invalid-body" })
	});
	let rawFormMethod = opts.formMethod || "get";
	let formMethod = normalizeFormMethod ? rawFormMethod.toUpperCase() : rawFormMethod.toLowerCase();
	let formAction = stripHashFromPath$1(path);
	if (opts.body !== void 0) {
		if (opts.formEncType === "text/plain") {
			if (!isMutationMethod$1(formMethod)) return getInvalidBodyError();
	...
	let formData;
	if (opts.formData) {
		searchParams = convertFormDataToSearchParams$1(opts.formData);
		formData = opts.formData;
	} else if (opts.body instanceof FormData) {
		searchParams = convertFormDataToSearchParams$1(opts.body);
		formData = opts.body;
	} else if (opts.body instanceof URLSearchParams) {
		searchParams = opts.body;
		formData = convertSearchParamsToFormData$1(searchParams);
	...
	if (isMutationMethod$1(submission.formMethod)) return {
		path,
		submission
	};
	let parsedPath = parsePath(path);
	if (isFetcher && parsedPath.search && hasNakedIndexQuery$1(parsedPath.search)) searchParams.append("index", "");
...
function getLoaderMatchesUntilBoundary$1(matches, boundaryId, includeBoundary) {
	if (includeBoundary === void 0) includeBoundary = false;
	let index = matches.findIndex((m) => m.route.id === boundaryId);
	if (index >= 0) return matches.slice(0, includeBoundary ? index + 1 : index);
	return matches;
}
function getMatchesToLoad$1(history, state, matches, submission, location, initialHydration, skipActionErrorRevalidation, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, deletedFetchers, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionResult) {
	let actionResult = pendingActionResult ? isErrorResult$1(pendingActionResult[1]) ? pendingActionResult[1].error : pendingActionResult[1].data : void 0;
	let currentUrl = history.createURL(state.location);
	let nextUrl = history.createURL(location);
	let boundaryMatches = matches;
	if (initialHydration && state.errors) boundaryMatches = getLoaderMatchesUntilBoundary$1(matches, Object.keys(state.errors)[0], true);
	else if (pendingActionResult && isErrorResult$1(pendingActionResult[1])) boundaryMatches = getLoaderMatchesUntilBoundary$1(matches, pendingActionResult[0]);
	...
	let navigationMatches = boundaryMatches.filter((match, index) => {
		...
		if (initialHydration) return shouldLoadRouteOnHydration$1(route, state.loaderData, state.errors);
		if (isNewLoader$1(state.loaderData, state.matches[index], match) || cancelledDeferredRoutes.some((id) => id === match.route.id)) return true;
		let currentRouteMatch = state.matches[index];
		let nextRouteMatch = match;
		return shouldRevalidateLoader$1(match, _extends$3({
			currentUrl,
			...
			actionResult,
			actionStatus,
			defaultShouldRevalidate: shouldSkipRevalidation ? false : isRevalidationRequired || currentUrl.pathname + currentUrl.search === nextUrl.pathname + nextUrl.search || currentUrl.search !== nextUrl.search || isNewRouteInstance$1(currentRouteMatch, nextRouteMatch)
		}));
	});
	...
	fetchLoadMatches.forEach((f, key) => {
		...
		let fetcher = state.fetchers.get(key);
		let fetcherMatch = getTargetMatch$1(fetcherMatches, f.path);
		...
		else shouldRevalidate = shouldRevalidateLoader$1(fetcherMatch, _extends$3({
			currentUrl,
...
function shouldLoadRouteOnHydration$1(route, loaderData, errors) {
	if (route.lazy) return true;
...
function isNewLoader$1(currentLoaderData, currentMatch, match) {
	let isNew = !currentMatch || match.route.id !== currentMatch.route.id;
	let isMissingData = currentLoaderData[match.route.id] === void 0;
	return isNew || isMissingData;
}
function isNewRouteInstance$1(currentMatch, match) {
...
function shouldRevalidateLoader$1(loaderMatch, arg) {
...
function patchRoutesImpl$1(routeId, children, routesToUse, manifest, mapRouteProperties) {
	...
	let newRoutes = convertRoutesToDataRoutes$1(children.filter((newRoute) => !childrenToPatch.some((existingRoute) => isSameRoute$1(newRoute, existingRoute))), mapRouteProperties, [
		routeId || "_",
		"patch",
...
function isSameRoute$1(newRoute, existingRoute) {
	...
	return newRoute.children.every((aChild, i) => {
		...
		return (_existingRoute$childr = existingRoute.children) == null ? void 0 : _existingRoute$childr.some((bChild) => isSameRoute$1(aChild, bChild));
	});
}
/**
...
*/
async function loadLazyRouteModule$1(route, mapRouteProperties, manifest) {
	...
	for (let lazyRouteProperty in lazyRoute) {
		...
		if (!isPropertyStaticallyDefined && !immutableRouteKeys$1.has(lazyRouteProperty)) routeUpdates[lazyRouteProperty] = lazyRoute[lazyRouteProperty];
	}
	Object.assign(routeToUpdate, routeUpdates);
	Object.assign(routeToUpdate, _extends$3({}, mapRouteProperties(routeToUpdate), { lazy: void 0 }));
}
async function defaultDataStrategy$1(_ref4) {
	let { matches } = _ref4;
...
async function callDataStrategyImpl$1(dataStrategyImpl, type, state, request, matchesToLoad, matches, fetcherKey, manifest, mapRouteProperties, requestContext) {
	let loadRouteDefinitionsPromises = matches.map((m) => m.route.lazy ? loadLazyRouteModule$1(m.route, mapRouteProperties, manifest) : void 0);
	let results = await dataStrategyImpl({
		matches: matches.map((match, i) => {
			...
			let resolve = async (handlerOverride) => {
				...
				return shouldLoad ? callLoaderOrAction$1(type, request, match, loadRoutePromise, handlerOverride, requestContext) : Promise.resolve({
...
async function callLoaderOrAction$1(type, request, match, loadRoutePromise, handlerOverride, staticContext) {
	let result;
	let onReject;
	...
	try {
		...
		if (loadRoutePromise) if (handler) {
			...
			if (handler) result = await runHandler(handler);
				...
				throw getInternalRouterError$1(405, {
					method: request.method,
					pathname,
			...
			let url = new URL(request.url);
			throw getInternalRouterError$1(404, { pathname: url.pathname + url.search });
...
async function convertDataStrategyResultToDataResult$1(dataStrategyResult) {
	...
	if (isResponse$1(result)) {
		...
		if (type === ResultType$1.error) return {
			type: ResultType$1.error,
			error: new ErrorResponseImpl$1(result.status, result.statusText, data),
	...
	if (type === ResultType$1.error) {
		if (isDataWithResponseInit$1(result)) {
			...
			return {
				type: ResultType$1.error,
				error: new ErrorResponseImpl$1(((_result$init3 = result.init) == null ? void 0 : _result$init3.status) || 500, void 0, result.data),
...
function normalizeRelativeRoutingRedirectResponse$1(response, request, routeId, matches, basename, v7_relativeSplatPath) {
	let location = response.headers.get("Location");
	invariant$1(location, "Redirects returned/thrown from loaders/actions must have a Location header");
	if (!ABSOLUTE_URL_REGEX$4.test(location)) {
		let trimmedMatches = matches.slice(0, matches.findIndex((m) => m.route.id === routeId) + 1);
		location = normalizeTo$1(new URL(request.url), trimmedMatches, basename, true, location, v7_relativeSplatPath);
...
function createClientSideRequest$1(history, location, signal, submission) {
	let url = history.createURL(stripHashFromPath$1(location)).toString();
	let init = { signal };
	if (submission && isMutationMethod$1(submission.formMethod)) {
		...
		else if (formEncType === "application/x-www-form-urlencoded" && submission.formData) init.body = convertFormDataToSearchParams$1(submission.formData);
		else init.body = submission.formData;
	}
	return new Request(url, init);
}
function convertFormDataToSearchParams$1(formData) {
	let searchParams = new URLSearchParams();
	for (let [key, value] of formData.entries()) searchParams.append(key, typeof value === "string" ? value : value.name);
	return searchParams;
}
function convertSearchParamsToFormData$1(searchParams) {
	let formData = new FormData();
	for (let [key, value] of searchParams.entries()) formData.append(key, value);
	return formData;
}
function processRouteLoaderData$1(matches, results, pendingActionResult, activeDeferreds, skipLoaderErrorBubbling) {
	let loaderData = {};
	let errors = null;
	let statusCode;
	let foundError = false;
	let loaderHeaders = {};
	let pendingError = pendingActionResult && isErrorResult$1(pendingActionResult[1]) ? pendingActionResult[1].error : void 0;
	matches.forEach((match) => {
		if (!(match.route.id in results)) return;
		let id = match.route.id;
		let result = results[id];
		invariant$1(!isRedirectResult$1(result), "Cannot handle redirect results in processLoaderData");
		if (isErrorResult$1(result)) {
			...
			if (skipLoaderErrorBubbling) errors[id] = error;
				...
				let boundaryMatch = findNearestBoundary$1(matches, id);
		...
		} else if (isDeferredResult$1(result)) {
...
function processLoaderData$1(state, matches, results, pendingActionResult, revalidatingFetchers, fetcherResults, activeDeferreds) {
	let { loaderData, errors } = processRouteLoaderData$1(matches, results, pendingActionResult, activeDeferreds, false);
	revalidatingFetchers.forEach((rf) => {
		let { key, match, controller } = rf;
		let result = fetcherResults[key];
		invariant$1(result, "Did not find corresponding fetcher result");
		if (controller && controller.signal.aborted) return;
		else if (isErrorResult$1(result)) {
			let boundaryMatch = findNearestBoundary$1(state.matches, match == null ? void 0 : match.route.id);
			if (!(errors && errors[boundaryMatch.route.id])) errors = _extends$3({}, errors, { [boundaryMatch.route.id]: result.error });
			state.fetchers.delete(key);
		} else if (isRedirectResult$1(result)) invariant$1(false, "Unhandled fetcher revalidation redirect");
		else if (isDeferredResult$1(result)) invariant$1(false, "Unhandled fetcher deferred data");
		else {
			let doneFetcher = getDoneFetcher$1(result.data);
			state.fetchers.set(key, doneFetcher);
		}
	});
	return {
...
function mergeLoaderData$1(loaderData, newLoaderData, matches, errors) {
...
function getActionDataForCommit$1(pendingActionResult) {
	if (!pendingActionResult) return {};
	return isErrorResult$1(pendingActionResult[1]) ? { actionData: {} } : { actionData: { [pendingActionResult[0]]: pendingActionResult[1].data } };
}
function findNearestBoundary$1(matches, routeId) {
...
function getShortCircuitMatches$1(routes) {
...
function getInternalRouterError$1(status, _temp5) {
	...
	return new ErrorResponseImpl$1(status || 500, statusText, new Error(errorMessage), true);
}
function findRedirect$1(results) {
	let entries = Object.entries(results);
	for (let i = entries.length - 1; i >= 0; i--) {
		let [key, result] = entries[i];
		if (isRedirectResult$1(result)) return {
			key,
			result
		};
	}
}
function stripHashFromPath$1(path) {
	let parsedPath = typeof path === "string" ? parsePath(path) : path;
	return createPath(_extends$3({}, parsedPath, { hash: "" }));
}
function isHashChangeOnly$1(a, b) {
...
function isRedirectDataStrategyResultResult$1(result) {
	return isResponse$1(result.result) && redirectStatusCodes$1.has(result.result.status);
}
function isDeferredResult$1(result) {
	return result.type === ResultType$1.deferred;
}
function isErrorResult$1(result) {
	return result.type === ResultType$1.error;
}
function isRedirectResult$1(result) {
	return (result && result.type) === ResultType$1.redirect;
}
function isDataWithResponseInit$1(value) {
...
function isValidMethod$1(method) {
	return validRequestMethods$1.has(method.toLowerCase());
}
function isMutationMethod$1(method) {
	return validMutationMethods$1.has(method.toLowerCase());
}
async function resolveNavigationDeferredResults$1(matches, results, signal, currentMatches, currentLoaderData) {
	let entries = Object.entries(results);
	for (let index = 0; index < entries.length; index++) {
		...
		let isRevalidatingLoader = currentMatch != null && !isNewRouteInstance$1(currentMatch, match) && (currentLoaderData && currentLoaderData[match.route.id]) !== void 0;
		if (isDeferredResult$1(result) && isRevalidatingLoader) await resolveDeferredData$1(result, signal, false).then((result) => {
			if (result) results[routeId] = result;
		});
	}
}
async function resolveFetcherDeferredResults$1(matches, results, revalidatingFetchers) {
	for (let index = 0; index < revalidatingFetchers.length; index++) {
		let { key, routeId, controller } = revalidatingFetchers[index];
		let result = results[key];
		if (!matches.find((m) => (m == null ? void 0 : m.route.id) === routeId)) continue;
		if (isDeferredResult$1(result)) {
			invariant$1(controller, "Expected an AbortController for revalidating fetcher deferred result");
			await resolveDeferredData$1(result, controller.signal, true).then((result) => {
				if (result) results[key] = result;
			});
		}
	}
}
async function resolveDeferredData$1(result, signal, unwrap) {
...
function hasNakedIndexQuery$1(search) {
	return new URLSearchParams(search).getAll("index").some((v) => v === "");
}
function getTargetMatch$1(matches, location) {
	let search = typeof location === "string" ? parsePath(location).search : location.search;
	if (matches[matches.length - 1].route.index && hasNakedIndexQuery$1(search || "")) return matches[matches.length - 1];
	let pathMatches = getPathContributingMatches$1(matches);
	return pathMatches[pathMatches.length - 1];
}
function getSubmissionFromNavigation$1(navigation) {
...
function getLoadingNavigation$1(location, submission) {
...
function getSubmittingNavigation$1(location, submission) {
	return {
		state: "submitting",
		location,
		formMethod: submission.formMethod,
...
function getLoadingFetcher$1(submission, data) {
	if (submission) return {
		state: "loading",
		formMethod: submission.formMethod,
		formAction: submission.formAction,
...
function getSubmittingFetcher$1(submission, existingFetcher) {
	return {
		state: "submitting",
		formMethod: submission.formMethod,
		formAction: submission.formAction,
...
function getDoneFetcher$1(data) {
	return {
		state: "idle",
		formMethod: void 0,
		formAction: void 0,
...
function restoreAppliedTransitions$1(_window, transitions) {
	try {
		let sessionPositions = _window.sessionStorage.getItem(TRANSITIONS_STORAGE_KEY$1);
		if (sessionPositions) {
...
function persistAppliedTransitions$1(_window, transitions) {
	if (transitions.size > 0) {
		let json = {};
		for (let [k, v] of transitions) json[k] = [...v];
		try {
			_window.sessionStorage.setItem(TRANSITIONS_STORAGE_KEY$1, JSON.stringify(json));
		} catch (error) {
...
function useHref(to, _temp) {
	...
	!useInRouterContext() && invariant$1(false, "useHref() may be used only in the context of a <Router> component.");
	let { basename, navigator } = import_react.useContext(NavigationContext);
	let { hash, pathname, search } = useResolvedPath(to, { relative });
	let joinedPathname = pathname;
	if (basename !== "/") joinedPathname = pathname === "/" ? basename : joinPaths$1([basename, pathname]);
...
function useLocation() {
	!useInRouterContext() && invariant$1(false, "useLocation() may be used only in the context of a <Router> component.");
...
function useMatch(pattern) {
	!useInRouterContext() && invariant$1(false, "useMatch() may be used only in the context of a <Router> component.");
	let { pathname } = useLocation();
	return import_react.useMemo(() => matchPath(pattern, decodePath$1(pathname)), [pathname, pattern]);
}
/**
...
function useNavigateUnstable() {
	!useInRouterContext() && invariant$1(false, "useNavigate() may be used only in the context of a <Router> component.");
	...
	let routePathnamesJson = JSON.stringify(getResolveToMatches$1(matches, future.v7_relativeSplatPath));
	...
	return import_react.useCallback(function(to, options) {
		...
		let path = resolveTo$1(to, JSON.parse(routePathnamesJson), locationPathname, options.relative === "path");
		if (dataRouterContext == null && basename !== "/") path.pathname = path.pathname === "/" ? basename : joinPaths$1([basename, path.pathname]);
...
function useResolvedPath(to, _temp2) {
	...
	let { pathname: locationPathname } = useLocation();
	let routePathnamesJson = JSON.stringify(getResolveToMatches$1(matches, future.v7_relativeSplatPath));
	return import_react.useMemo(() => resolveTo$1(to, JSON.parse(routePathnamesJson), locationPathname, relative === "path"), [
		to,
		routePathnamesJson,
		locationPathname,
		relative
...
function useRoutesImpl(routes, locationArg, dataRouterState, future) {
	!useInRouterContext() && invariant$1(false, "useRoutes() may be used only in the context of a <Router> component.");
	...
	let renderedMatches = _renderMatches(matches && matches.map((match) => Object.assign({}, match, {
		...
		pathname: joinPaths$1([parentPathnameBase, navigator.encodeLocation ? navigator.encodeLocation(match.pathname).pathname : match.pathname]),
		pathnameBase: match.pathnameBase === "/" ? parentPathnameBase : joinPaths$1([parentPathnameBase, navigator.encodeLocation ? navigator.encodeLocation(match.pathnameBase).pathname : match.pathnameBase])
...
function _renderMatches(matches, parentMatches, dataRouterState, future) {
	...
	if (errors != null) {
		...
		!(errorIndex >= 0) && invariant$1(false, "Could not find a matching route for errors on route IDs: " + Object.keys(errors).join(","));
...
function useDataRouterContext$1(hookName) {
	...
	!ctx && invariant$1(false, getDataRouterConsoleError$1(hookName));
	return ctx;
}
function useDataRouterState$1(hookName) {
	let state = import_react.useContext(DataRouterStateContext);
	!state && invariant$1(false, getDataRouterConsoleError$1(hookName));
	return state;
}
function useRouteContext(hookName) {
	let route = import_react.useContext(RouteContext);
	!route && invariant$1(false, getDataRouterConsoleError$1(hookName));
	return route;
}
function useCurrentRouteId(hookName) {
	let route = useRouteContext(hookName);
	let thisRoute = route.matches[route.matches.length - 1];
	!thisRoute.route.id && invariant$1(false, hookName + " can only be used on routes that contain a unique \"id\"");
...
function useMatches() {
	...
	return import_react.useMemo(() => matches.map((m) => convertRouteMatchToUiMatch$1(m, loaderData)), [matches, loaderData]);
}
/**
...
function useBlocker(shouldBlock) {
	...
	let blockerFunction = import_react.useCallback((arg) => {
		...
		return shouldBlock({
			currentLocation: _extends$2({}, currentLocation, { pathname: stripBasename$1(currentLocation.pathname, basename) || currentLocation.pathname }),
			nextLocation: _extends$2({}, nextLocation, { pathname: stripBasename$1(nextLocation.pathname, basename) || nextLocation.pathname }),
			historyAction
		});
	...
	return blockerKey && state.blockers.has(blockerKey) ? state.blockers.get(blockerKey) : IDLE_BLOCKER$1;
}
/**
...
function useNavigateStable() {
	...
	return import_react.useCallback(function(to, options) {
		...
		else router.navigate(to, _extends$2({ fromRouteId: id }, options));
	}, [router, id]);
}
var alreadyWarned$1 = {};
...
function Navigate(_ref4) {
	...
	!useInRouterContext() && invariant$1(false, "<Navigate> may be used only in the context of a <Router> component.");
	...
	let path = resolveTo$1(to, getResolveToMatches$1(matches, future.v7_relativeSplatPath), locationPathname, relative === "path");
...
function Router(_ref5) {
	...
	useInRouterContext() && invariant$1(false, "You cannot render a <Router> inside another <Router>. You should never have more than one in your app.");
	let basename = basenameProp.replace(/^\/*/, "/");
	let navigationContext = import_react.useMemo(() => ({
		basename,
		navigator,
		static: staticProp,
		future: _extends$2({ v7_relativeSplatPath: false }, future)
	}), [
		basename,
		future,
		navigator,
...
function createRoutesFromChildren(children, parentPath) {
	...
	import_react.Children.forEach(children, (element, index) => {
		...
		!(element.type === Route) && invariant$1(false, "[" + (typeof element.type === "string" ? element.type : element.type.name) + "] is not a <Route> component. All component children of <Routes> must be a <Route> or <React.Fragment>");
		!(!element.props.index || !element.props.children) && invariant$1(false, "An index route cannot have child routes.");
		let route = {
...
function createMemoryRouter(routes, opts) {
	return createRouter$1({
		...
		future: _extends$2({}, opts == null ? void 0 : opts.future, { v7_prependBasename: true }),
...
//#endregion
//#region node_modules/react-router-dom/node_modules/@remix-run/router/dist/router.js
/**
* @remix-run/router v1.23.2
*
* Copyright (c) Remix Software Inc.
...
function createHashHistory(options) {
	...
	function createHashLocation(window, globalHistory) {
		let { pathname = "/", search = "", hash = "" } = parsePath$1(window.location.hash.substr(1));
	...
	function createHashHref(window, to) {
		...
		return href + "#" + (typeof to === "string" ? to : createPath$1(to));
	}
...
function convertRoutesToDataRoutes(routes, mapRouteProperties, parentPath, manifest) {
	...
	return routes.map((route, index) => {
		...
		if (isIndexRoute(route)) {
			let indexRoute = _extends$1({}, route, mapRouteProperties(route), { id });
			manifest[id] = indexRoute;
			return indexRoute;
		} else {
...
* @see https://reactrouter.com/v6/utils/match-routes
*/
function matchRoutes$1(routes, locationArg, basename) {
	if (basename === void 0) basename = "/";
...
function matchRouteBranch(branch, pathname, allowPartial) {
	...
	for (let i = 0; i < routesMeta.length; ++i) {
		...
		let end = i === routesMeta.length - 1;
		let remainingPathname = matchedPathname === "/" ? pathname : pathname.slice(matchedPathname.length) || "/";
		let match = matchPath$1({
			path: meta.relativePath,
			caseSensitive: meta.caseSensitive,
			end
		}, remainingPathname);
		let route = meta.route;
		if (!match && end && allowPartial && !routesMeta[routesMeta.length - 1].route.index) match = matchPath$1({
			path: meta.relativePath,
			caseSensitive: meta.caseSensitive,
			end: false
		}, remainingPathname);
...
function resolveTo(toArg, routePathnames, locationPathname, isPathRelative) {
	...
	let path = resolvePath$1(to, from);
	let hasExplicitTrailingSlash = toPathname && toPathname !== "/" && toPathname.endsWith("/");
	let hasCurrentTrailingSlash = (isEmptyPath || toPathname === ".") && locationPathname.endsWith("/");
	if (!path.pathname.endsWith("/") && (hasExplicitTrailingSlash || hasCurrentTrailingSlash)) path.pathname += "/";
	return path;
...
function createRouter(init) {
	...
	let getScrollPosition = null;
	let initialScrollRestored = init.hydrationData != null;
	let initialMatches = matchRoutes$1(dataRoutes, init.history.location, basename);
	let initialMatchesIsFOW = false;
	let initialErrors = null;
	if (initialMatches == null && !patchRoutesOnNavigationImpl) {
	...
	function initialize() {
		...
		if (!state.initialized) startNavigation(Action$1.Pop, state.location, { initialHydration: true });
		return router;
	}
	function dispose() {
		if (unlistenHistory) unlistenHistory();
	...
	function completeNavigation(location, newState, _temp) {
		...
		else if (pendingAction === Action$1.Push) init.history.push(location, location.state);
		else if (pendingAction === Action$1.Replace) init.history.replace(location, location.state);
		let viewTransitionOpts;
	...
	async function navigate(to, opts) {
		...
		if (userReplace === true) historyAction = Action$1.Replace;
		else if (userReplace === false);
		else if (submission != null && isMutationMethod(submission.formMethod) && submission.formAction === state.location.pathname + state.location.search) historyAction = Action$1.Replace;
		let preventScrollReset = opts && "preventScrollReset" in opts ? opts.preventScrollReset === true : void 0;
		let flushSync = (opts && opts.flushSync) === true;
		let blockerKey = shouldBlockNavigation({
			currentLocation,
	...
	async function startNavigation(historyAction, location, opts) {
		...
		let routesToUse = inFlightDataRoutes || dataRoutes;
		let loadingNavigation = opts && opts.overrideNavigation;
		let matches = opts != null && opts.initialHydration && state.matches && state.matches.length > 0 && !initialMatchesIsFOW ? state.matches : matchRoutes$1(routesToUse, location, basename);
		let flushSync = (opts && opts.flushSync) === true;
		...
		if (opts && opts.pendingError) pendingActionResult = [findNearestBoundary(matches).route.id, {
			...
			if (actionResult.pendingActionResult) {
				let [routeId, result] = actionResult.pendingActionResult;
				if (isErrorResult(result) && isRouteErrorResponse$1(result.error) && result.error.status === 404) {
					pendingNavigationController = null;
					completeNavigation(location, {
						matches: actionResult.matches,
						loaderData: {},
	...
	async function handleAction(request, location, submission, matches, isFogOfWar, opts) {
		...
		if (isErrorResult(result)) {
			...
			if ((opts && opts.replace) !== true) pendingAction = Action$1.Push;
			return {
				matches,
				pendingActionResult: [boundaryMatch.route.id, result]
			};
	...
	function fetch(key, routeId, href, opts) {
		...
		let matches = matchRoutes$1(routesToUse, normalizedPath, basename);
		let fogOfWar = checkFogOfWar(matches, routesToUse, normalizedPath);
		if (fogOfWar.active && fogOfWar.matches) matches = fogOfWar.matches;
		if (!matches) {
	...
	async function handleFetcherAction(key, routeId, path, match, requestMatches, isFogOfWar, flushSync, preventScrollReset, submission) {
		...
		let routesToUse = inFlightDataRoutes || dataRoutes;
		let matches = state.navigation.state !== "idle" ? matchRoutes$1(routesToUse, state.navigation.location, basename) : state.matches;
	...
	*/
	...
	async function discoverRoutes(matches, pathname, signal, fetcherKey) {
		...
		while (true) {
			...
			if (signal.aborted) return { type: "aborted" };
			let newMatches = matchRoutes$1(routesToUse, pathname, basename);
			if (newMatches) return {
				type: "success",
				matches: newMatches
			};
...
function getMatchesToLoad(history, state, matches, submission, location, initialHydration, skipActionErrorRevalidation, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, deletedFetchers, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionResult) {
	...
	fetchLoadMatches.forEach((f, key) => {
		if (initialHydration || !matches.some((m) => m.route.id === f.routeId) || deletedFetchers.has(key)) return;
		let fetcherMatches = matchRoutes$1(routesToUse, f.path, basename);
		if (!fetcherMatches) {
			revalidatingFetchers.push({
				key,
				routeId: f.routeId,
...
async function loadLazyRouteModule(route, mapRouteProperties, manifest) {
	...
	Object.assign(routeToUpdate, routeUpdates);
	Object.assign(routeToUpdate, _extends$1({}, mapRouteProperties(routeToUpdate), { lazy: void 0 }));
}
	...
	if (type === ResultType.error) {
		if (isDataWithResponseInit(result)) {
			...
			return {
				...
				statusCode: isRouteErrorResponse$1(result) ? result.status : void 0,
				headers: (_result$init4 = result.init) != null && _result$init4.headers ? new Headers(result.init.headers) : void 0
			};
		}
		return {
...
function processRouteLoaderData(matches, results, pendingActionResult, activeDeferreds, skipLoaderErrorBubbling) {
	...
	matches.forEach((match) => {
		...
		if (isErrorResult(result)) {
			...
			if (!foundError) {
				foundError = true;
				statusCode = isRouteErrorResponse$1(result.error) ? result.error.status : 500;
			}
			if (result.headers) loaderHeaders[id] = result.headers;
...
function stripHashFromPath(path) {
	...
	return createPath$1(_extends$1({}, parsedPath, { hash: "" }));
}
...
function useViewTransitionState(to, opts) {
	...
	return matchPath$1(path.pathname, nextPath) != null || matchPath$1(path.pathname, currentPath) != null;
}
//#endregion
